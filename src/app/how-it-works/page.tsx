    ...
    <main className="bg-[#0A1422] text-white">
      ...
      <section id="flow" className="border-t border-white/5">
        <div className="mx-auto max-w-7xl px-6 lg:px-12 py-20">
          ...
          <ol className="grid lg:grid-cols-6 gap-5">
            {[
              { h: 'Tag → secure link', p: 'Each tag holds a unique, signed URL.' },
              { h: 'Identify context', p: 'Match tag, campaign, and location.' },
              { h: 'Launch mobile experience', p: 'Your branded page loads instantly.' },
              { h: 'Trigger actions', p: 'Join, check in, enter raffle, redeem.' },
              { h: 'Capture engagement data', p: 'Time, location, item, outcome.' },
              { h: 'Close the loop', p: 'Update rewards and sync to CRM.' },
            ].map((step, i) => (
              <li
                key={i}
                className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 flex flex-col gap-3"
              >
                <div className="aspect-[16/10] rounded-xl overflow-hidden border border-white/10 bg-black/30">
                  <Image
                    src="/images/placeholders/how-it-works/flow-node.svg"
                    alt=""
                    width={1200}
                    height={750}
                    className="w-full h-full object-cover opacity-90"
                ...
                <div className="text-lg font-medium">{step.h}</div>
                <p className="text-white/75 text-sm">{step.p}</p>
              </li>
            ))}
          </ol>
        </div>
