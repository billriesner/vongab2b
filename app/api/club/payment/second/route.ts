...
export async function POST(request: NextRequest) {
  try {
    ...
    if (order['Payment Status'] !== 'Second Payment Due') {
      return NextResponse.json(
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `40% payment for ${order['Organization Name']} order (${order['Total Units']} units)`,
            },
            unit_amount: Math.round(order['Second Payment Amount'] * 100), // Convert to cents
          },
          quantity: 1,
        },
      ],
      ...
      ui_mode: 'hosted',
      metadata: {
        type: 'club_second_payment',
        orderId: orderId,
        paymentType: 'second_payment',
        organizationName: order['Organization Name'],
        customerEmail: order['Email'],
        amount: order['Second Payment Amount'].toString(),
    ...
    await updateOrder(orderId, {
      'Second Payment Intent ID': session.id
    });
