...
interface TermsContactFormProps {
  className?: string;
}

export function TermsContactForm({ className = '' }: TermsContactFormProps) {
    ...
    try {
      const response = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          name,
          email,
          company: 'Terms Question', // Auto-fill company for Terms questions
          message,
        }),
      });
    ...
    <form onSubmit={handleSubmit} className={className}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <textarea
          ...
          rows={4}
          className="w-full px-lg py-md border-2 border-muted rounded focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent text-navy bg-white"
          placeholder="Ask us anything about our Terms & Conditions..."
          value={message}
